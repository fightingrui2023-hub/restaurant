...
export default function Stack({
  ...
  const sendToBack = id => {
    setStack(prev => {
      ...
      // Move current card to the end so cards advance in forward order.
      newStack.push(card);
      return newStack;
    });
  };
  ...
  useEffect(() => {
    if (!stack.length || !onActiveIndexChange) return;
    const topCard = stack[0];
    if (!topCard) return;
    onActiveIndexChange(topCard.id - 1);
  }, [stack, onActiveIndexChange]);
